  ...
  return (
    <Container maxW="87%" mx="auto" position="relative">
      ...
      >
        {categorys.map((c, i) => (
          <Link  href="" key={i}>
            <Box  onClick={()=>navigate(`/products/search?category=${c.name}&&value=`)}  cursor="pointer"  h="185px" borderWidth="1px" p={3}>
              <Image
                src={c.image}
                alt={`Category ${c}`}
                w="full"
              ...
              <Flex
                ...
                fontWeight="bold"
              >
                <Text py="2px" px={6} bg="rgba(51, 48, 48, 0.36)" color="white">
                  {c.name}
                </Text>
              </Flex>
            </Box>
